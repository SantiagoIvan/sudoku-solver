...
def sudoku(puzzle):
    ...
    if coords:
        (row, column) = coords
        for n in range(1,10):
            if (not is_in_the_row(puzzle, row, n)) and (not is_in_the_column(puzzle, column, n))  and (not is_in_the_region(puzzle, row, column, n)):
                # si el numero en cuestion es un numero valido, lo inserto
                puzzle[row][column] = n
                if sudoku(puzzle): return puzzle # si alguno de mis hijos me retorna un puzzle, significa que lo pude terminar y lo retorno
